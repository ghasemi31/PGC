...
{
    public class GameManagerBusiness:BaseEntityManagementBusiness<User,pgcEntities>
    {
        public GameManagerBusiness()
        {
            Context = new pgcEntities();
        }

        public IQueryable Search_Select(int startRowIndex, int maximumRows, GameManagerPattern Pattern)
        {
        ...
        public int Search_Count(GameManagerPattern Pattern)
        {
            return Search_Where(Context.Users, Pattern).Count();
        }

        public IQueryable<User> Search_Where(IQueryable<User> list, GameManagerPattern Pattern)
        {
            list = list.Where(f => f.IsGameManager);
            //DefaultPattern
            if (Pattern == null)
                return list;
        ...
        public List<User> Search_Select(GameManagerPattern Pattern)
        {
            var Result = Search_Where(Context.Users, Pattern)
                .OrderByDescending(f => f.SignUpPersianDate);
