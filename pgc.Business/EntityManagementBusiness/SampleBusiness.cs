...
{
    public class SampleBusiness:BaseEntityManagementBusiness<Sample,pgcEntities>
    {
        ...
        public IQueryable Search_Select(int startRowIndex, int maximumRows, SamplePattern Pattern)
        {
            ...
            var Result = Search_Where(Context.Samples, Pattern)
                ...
                .Select(f => new
                {
                    ...
                    f.EndPersianDate,
                    f.Description,
                    IsDoneState = f.IsDone ? "به اتمام رسیده" : "به اتمام نرسیده"
                });

            return Result.Skip(startRowIndex).Take(maximumRows);
        }

        public int Search_Count(SamplePattern Pattern)
        {
            return Search_Where(Context.Samples, Pattern).Count();
        }

        public IQueryable<Sample> Search_Where(IQueryable<Sample> list, SamplePattern Pattern)
        {
            //DefaultPattern
            if (Pattern == null)
                return list;
