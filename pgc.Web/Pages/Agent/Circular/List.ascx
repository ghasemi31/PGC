<%@ Control Language="C#" AutoEventWireup="true" CodeFile="List.ascx.cs" Inherits="Pages_Agent_Circular_List" %>
...
<kfk:HKGrid ID="grdList"
    ...
    <Columns>
        ...
        <kfk:TextColumnTemplate DataField="Title" HeaderText="عنوان بخشنامه" />
        <asp:TemplateField HeaderText="تاریخ دریافت">
            <ItemTemplate>
                 <span><%--<%#kFrameWork.Util.DateUtil.GetPersianDateWithTime(Convert.ToDateTime(Eval("Date"))) %>--%></span>          
            </ItemTemplate>
        </asp:TemplateField>
        <kfk:BaseButtonField ButtonType="Button" CommandName="EditRow" Text="نمایش" />
    </Columns>
</kfk:HKGrid>
