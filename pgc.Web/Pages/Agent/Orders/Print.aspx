...
<body>
    <form id="form1" runat="server">
        <div>
              ...
              <%try
                {
                    OrdersAgentPattern pattern = (OrdersAgentPattern)Session["OrderPattern"];
                    OrdersAgentBusiness business = new OrdersAgentBusiness();
                    ...
                    foreach (var item in OrdersList)
                    {
                        ...
                        <tr class="<%=(isAltRow)?"altrow":"row" %>">
                            ...
                            <td><% if (item.PaymentType == (int)PaymentType.Online)
                                   {
                                       bool isPaid = item.OnlinePayments.Any(g => g.TransactionState == "OK" && g.ResultTransaction > 0 && g.RefNum != "");
                                       if (isPaid)
                                       {%>
                                                <span class="ordsts">آنلاین (<%=EnumUtil.GetEnumElementPersianTitle(OrderPaymentStatus.OnlineSucced)%>)</span>
                                            <% }
                                       else
                                       {%>آنلاین (<%=EnumUtil.GetEnumElementPersianTitle(OrderPaymentStatus.OnlineFailed)%>)<% }
                                   }
                                   else
                                    %><%=EnumUtil.GetEnumElementPersianTitle(PaymentType.Presence)%>
                            </td>
                    ...
                    <%
                        ...
                        {%>
                                <tr class="ftr">
                                    <td colspan="6">جمع کل: <%=UIUtil.GetCommaSeparatedOf(TotalAmount)%> ریال</td>
                                </tr>
                            </table>
                        <%}
