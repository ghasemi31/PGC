<%@ Control Language="C#" AutoEventWireup="true" CodeFile="Search.ascx.cs" Inherits="Pages_Admin_GalleryPic_Search" %>
<legend>جستجو</legend>
<table>
    <tr>
        <td class="caption">گالری</td>
        <td class="control"><kfk:LookupCombo ID="lkpGallery" 
                                            runat="server" 
                                            BusinessTypeName="pgc.Business.Lookup.GalleryLookupBusiness"
                                            AddDefaultItem="true" /></td>
    </tr>
    <tr>
        <td class="caption">توضیحات</td>
        <td class="control"><kfk:NormalTextBox ID="txtDesc" runat="server" /></td>
    </tr>
</table>
<div class="commands">
    <asp:Button runat="server" ID="btnSearch" Text="جستجو" CssClass="" OnClick="OnSearch" />
    <asp:Button runat="server" ID="btnShowAll" Text="نمایش همه" CssClass="" OnClick="OnSearchAll" />
</div>
