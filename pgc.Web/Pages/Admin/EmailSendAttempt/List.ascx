<%@ Control Language="C#" AutoEventWireup="true" CodeFile="List.ascx.cs" Inherits="Pages_Admin_EmailSendAttempt_List" %>
<legend><%=(this.Page as kFrameWork.UI.BasePage).Entity.Title %></legend>
<div class="list-help">
    <div class="commands">
        <asp:Button Visible="false" ID="btnBulkDelete" runat="server" Text="حذف ایمیل های انتخاب شده" OnClick="OnBulkDelete" Width="160" OnClientClick="if (!confirm('آیا در حذف کلیه سطرهای انتخاب شده ، اطمینان دارید؟')){return false;}" />
...
<kfk:HKGrid ID="grdList"
    ...
    <Columns>
        ...
        <%--<kfk:TextColumnTemplate DataField="Subject" HeaderText="عنوان ایمیل" MaxLength="20"/>--%>
        <kfk:TextColumnTemplate DataField="SentEmail_Count" HeaderText="تعداد ارسالی" />
        <kfk:TextColumnTemplate DataField="TotalEmail_Count" HeaderText="تعداد کل" />
