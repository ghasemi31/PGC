...
(function ($) {
    ...
    $.widget('blueimpUI.fileupload', $.blueimp.fileupload, {
        ...
        options: {
            ...
            add: function (e, data) {
                //new-----------------------------
                $('.fileinput-button').fadeOut();
                //new-----------------------------
                //try { parent.hideValidator($('#hdf_contId').val()); } catch (err) { }
                ...
                if (data.context) {
                    data.context.each(function (index) {
                        ...
                        $(this).fadeOut(function () {
                            var path = $(this).parent().find('tr.template-download td.name a').text();
                            that._renderDownload([file])
                                .css('display', 'none')
                                .replaceAll(this);
                            window.parent.$('#' + $('#hdf_contId').val()).find('.hdf_FilePath').val(path);
                        });
                    });
                } else {
            ...
            fail: function (e, data) {
                //try { parent.showValidator($('#hdf_contId').val()); } catch (err) { }
                //new-----------------------------
                $('.fileinput-button').fadeIn();
                //new-----------------------------
                var that = $(this).data('fileupload');
                that._adjustMaxNumberOfFiles(data.files.length);
                if (data.context) {
            ...
            destroy: function (e, data) {
                ...
                var path = $('.files table:last td.name a').text();
                var page = 'DeleteFile.aspx';
...
//                try { parent.path_changed('', $('#hdf_contId').val()); } catch (err) { }
                try { window.parent.$('#' + $('#hdf_contId').val()).find('.hdf_FilePath').val(path); } catch (err) { }
                //new-----------------------------
                $('.fileinput-button').fadeIn();
                //-----------------------------
